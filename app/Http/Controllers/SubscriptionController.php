...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create pricing packages')) {
            $validator = \Validator::make(
                $request->all(), [
                    'title' => 'required',
                    'package_amount' => isset($request->is_trial) && $request->is_trial ? 'nullable' : 'required',
                    'interval' => 'required',
                    'user_limit' => 'required',
            ...
            $subscription->is_trial = isset($request->is_trial) ? 1 : 0;
            if ($subscription->is_trial) {
                $subscription->interval = 'Trial';
                $subscription->package_amount = 0.00;
            } else {
    ...
    public function startTrial($ids)
    {
        if (\Auth::user()->can('buy pricing packages')) {
            $id = Crypt::decrypt($ids);
            $subscription = Subscription::find($id);
            ...
            if (!$subscription->is_trial) {
                return redirect()->route('subscriptions.index')->with('error', __('This is not a free package.'));
            }
            
            // Allow users to switch to free package even if they already have one
            // No need to check if they already have a free subscription
            
            // Assign trial subscription without payment
            ...
            if ($assignPlan['is_success']) {
                ...
                $data['payment_type'] = 'Free Trial';
                $data['status'] = 'Succeeded';
                PackageTransaction::transactionData($data);
                
                return redirect()->route('subscriptions.index')->with('success', __('Free package activated successfully. You can upgrade to premium anytime!'));
            } else {
    ...
    public function upgrade($ids)
    {
        if (\Auth::user()->can('buy pricing packages')) {
            ...
            // Allow upgrade from free package to paid packages
            // No restriction - users can upgrade anytime
