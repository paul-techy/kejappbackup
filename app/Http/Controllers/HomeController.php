        ...
        if (\Auth::check()) {
            ...
            if (!file_exists(setup())) {
                ...
                if ($landingPage == 'on') {
                    ...
                    // Get the cheapest monthly paid subscription for trial upgrade info
                    $cheapestPaidSubscription = Subscription::where('is_trial', 0)
                        ->where('package_amount', '>', 0)
                        ->where('interval', 'Monthly')
                        ->orderBy('package_amount', 'asc')
                        ->first();
                    // If no monthly plan, get any cheapest paid plan
                    if (!$cheapestPaidSubscription) {
                        $cheapestPaidSubscription = Subscription::where('is_trial', 0)
                            ->where('package_amount', '>', 0)
                            ->orderBy('package_amount', 'asc')
                            ->first();
                    ...
                    return view('layouts.landing', compact('subscriptions', 'menus', 'FAQs', 'cheapestPaidSubscription'));
                } else {
