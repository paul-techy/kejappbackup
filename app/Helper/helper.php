...
if (!function_exists('assignSubscription')) {
    function assignSubscription($id)
    {
        ...
        if ($subscription) {
            ...
            if ($subscription->interval == 'Trial' || $subscription->is_trial == 1) {
                // Lifetime free trial - no expiry date
                \Auth::user()->subscription_expire_date = null;
            } elseif ($subscription->interval == 'Monthly') {
...
if (!function_exists('send_zettatel_sms_file')) {
    /**
     * Send SMS to multiple recipients using Zettatel file upload method
     * 
     ...
     */
    function send_zettatel_sms_file($mobileNumbers, $message, $options = [])
    {
        try {
            $zettatelService = new \App\Services\ZettatelSmsService();
            
            if (!$zettatelService->isConfigured()) {
                return [
                    'status' => 'error',
                    'message' => 'Zettatel SMS is not configured. Please configure in settings.',
                ];
            }

            // Create CSV file with mobile numbers
            $filePath = $zettatelService->createMobileFile($mobileNumbers);

            // Send SMS
            $response = $zettatelService->sendSmsFile($filePath, $message, $options);

            // Clean up temporary file
            $zettatelService->cleanupFile($filePath);

            return $response;
        } catch (\Exception $e) {
            \Log::error('Zettatel SMS send failed: ' . $e->getMessage());
            return [
                'status' => 'error',
...
if (!function_exists('send_zettatel_sms')) {
    /**
     * Send SMS to a single recipient using Zettatel file upload method
     * This is a convenience wrapper for single recipient SMS
     * 
     * @param string $mobileNumber Single mobile number
     * @param string $message SMS message content
     * @param array $options Additional options
     * @return array Response array with status and message
     */
    function send_zettatel_sms($mobileNumber, $message, $options = [])
    {
        ...
        // Use the bulk function with single recipient
        return send_zettatel_sms_file([$mobileNumber], $message, $options);
    }
}
